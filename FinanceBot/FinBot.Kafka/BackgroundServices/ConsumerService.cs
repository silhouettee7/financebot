...
public class ConsumerService<TKey, TValue, THandler>(
    RegistrationConsumer<TKey, TValue, THandler> registrationConsumer,
    ...
    : BackgroundService where THandler : IMessageHandler<TKey, TValue>
{
    ...
    private readonly ConsumerSettings<THandler> _settings = registrationConsumer.Settings;
    private THandler _handler = registrationConsumer.Settings.Handler;
...
public class ConsumerService<TValue, THandler>(
    RegistrationConsumer<Null, TValue, THandler> registrationConsumer,
    ILogger<ConsumerService<TValue, THandler>> logger)
    : BackgroundService where THandler : IMessageHandler<TValue>
{
    private readonly IConsumer<Null, TValue> _consumer = registrationConsumer.Consumer;
    private readonly ConsumerSettings<THandler> _settings = registrationConsumer.Settings;
    private THandler _handler = registrationConsumer.Settings.Handler;
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                while (!success && retryCount < _settings.MaxRetryCount && !stoppingToken.IsCancellationRequested)
                {
                    try
                    {
                        await _handler.HandleAsync(consumeResult.Message.Value, stoppingToken);
                        success = true;
                    }
                    catch (Exception ex) when (retryCount < _settings.MaxRetryCount - 1)
                    {
    ...
    private async Task SendToDeadLetterQueue(ConsumeResult<Null, TValue> consumeResult, Exception exception, CancellationToken cancellationToken)
    {
        logger.LogInformation("Отправка сообщения в DLQ: {Exception}", exception.Message);
        await Task.CompletedTask;
    }
